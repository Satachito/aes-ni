...
extern "C" {
	...
	void AES_192_Key_Expansion( const unsigned char*, unsigned char* );
...
,	AES_KEY* key
	...
	if (bits == 128) {
		...
		AES_192_Key_Expansion (userKey,(unsigned char*)key);
		key->nr = 12;
		return 0;
	} else if (bits == 256) {
		AES_256_Key_Expansion (userKey,(unsigned char*)key);
		key->nr = 14;
		return 0;
	}
	return -2;
